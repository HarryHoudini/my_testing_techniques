...
public class EditingVirtualMemorySensor {
    ...
    private final VirtualMemoryData SENSOR_DATA = new VirtualMemoryData(Builder.of()
        .setCaption(SENSOR_CAPTION)
    );
    private final SensorType SENSOR_TYPE = SensorType.VIRTUAL_MEMORY;
    ...
    @DataSupplier
    public StreamEx<VirtualMemoryData> getPositiveData(){
        return StreamEx.of(
            StreamEx.of(DataConstants.REQ_POSITIVE_STRS)
                .map(string -> new VirtualMemoryData(Builder.of().setCaption(string)))
        ).append(
            StreamEx.of(DataConstants.PERCENT_POSITIVE_NUMS)
                .map(percent -> new VirtualMemoryData(Builder.of().setExpectedValue(percent)))
        ).append(
            StreamEx.of(DataConstants.POSITIVE_NUM_STRS)
                .map(string -> new VirtualMemoryData(Builder.of().setExpectedValueString(string)))
        ).append(
            StreamEx.of(VirtualMemoryData.ConditionType.values())
                .map(operator -> new VirtualMemoryData(Builder.of().setOperator(operator)))
        );
    }

    @DataSupplier
    public StreamEx<VirtualMemoryData> getNegativeData(){
        return StreamEx.of(
    ...
    public void shouldEditVirtualMemory(final VirtualMemoryData data){
    ...
    public void shouldNotEditVirtualMemory(final VirtualMemoryData data){
