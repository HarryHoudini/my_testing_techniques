...
public class EditingCpuUsageSensor {
    ...
    private final String SENSOR_CAPTION = "Cpu Usage sensor";
    private final CpuUsageData SENSOR_DATA = new CpuUsageData(Builder.of()
        .setCaption(SENSOR_CAPTION)
    );
    private final SensorType SENSOR_TYPE = SensorType.CPU_USAGE;
    ...
    private void networkTreeSetUp() {
        ...
        this.networkTreeWidget.getFooter().setCheckbox(NetworkTreeFooterCheckbox.UNDEFINED, true);
    ...
    @DataSupplier
    public StreamEx<CpuUsageData> getPositiveData(){
        return StreamEx.of(
            StreamEx.of(DataConstants.REQ_POSITIVE_STRS)
                .map(string -> new CpuUsageData(Builder.of().setCaption(string)))
        ).append(
            StreamEx.of(DataConstants.PERCENT_POSITIVE_NUMS)
                .map(percent -> new CpuUsageData(Builder.of().setExpectedValue(percent)))
        ).append(
            StreamEx.of(DataConstants.POSITIVE_NUM_STRS)
                .map(string -> new CpuUsageData(Builder.of().setExpectedValueString(string)))
        ).append(
            StreamEx.of(CpuUsageData.ConditionType.values())
                .map(operator -> new CpuUsageData(Builder.of().setOperator(operator)))
        );
    }

    @DataSupplier
    public StreamEx<CpuUsageData> getNegativeData(){
        return StreamEx.of(
    ...
    public void shouldEditCpuUsage(final CpuUsageData data){
    ...
    public void shouldNotEditCpuUsage(final CpuUsageData data){
